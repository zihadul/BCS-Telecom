...
public partial class Pages_User_MemberDirectory : System.Web.UI.Page
{

    bllMember objMember = new bllMember();

    private static int PageSize = 20;
    private const int pageDispCount = 5;
    ...
    public void BindData()
    {
        ...
        if (tbxMemberName.Text.Trim() != "")
            Criteria = "membername like '%" + tbxMemberName.Text.Trim() + "%'";

        if (tbxDesignation.Text.Trim() != "")
        {
            if (Criteria == "")
                Criteria = "designation like '%" + tbxDesignation.Text.Trim() + "%'";
            else
                Criteria += " And designation like '%" + tbxDesignation.Text.Trim() + "%'";
        }

        if (tbxOffice.Text.Trim() != "")
        {
            if (Criteria == "")
                Criteria = "nameoftheoffice like '%" + tbxOffice.Text.Trim() + "%'";
            else
                Criteria += " And nameoftheoffice like '%" + tbxOffice.Text.Trim() + "%'";
        }

        DataView dv = objMember.GetList(CurrentPage * PageSize, PageSize).DefaultView;
        dv.RowFilter = Criteria;
        DataTable dt = dv.ToTable();
        if (dt.Rows.Count > 0)
        {
            TotalRec = Convert.ToInt32(dt.Rows[0]["Total"]);
        }
        else
        {
            TotalRec = 0;
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        DataView dv = objMember.GetAllUser().DefaultView;
        dv.RowFilter = Criteria;
        DataTable dt = dv.ToTable();
        if (dt.Rows.Count > 0)
        {
    ...
    {
        Response.Redirect("~/Pages/User/MemberDirectoryPrint.aspx");
    }
