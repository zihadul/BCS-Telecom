...
public partial class Pages_Member_EditProfile : BasePage
{
    ...
    private void Populate()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            if (dt.Rows[0]["joindate"].ToString() != "" && Common.IsDate(dt.Rows[0]["joindate"].ToString()))
            {
    ...
    public void OnBtnSave_Click(object sender, EventArgs e)
    {
        ...
        objMember.Update(tbxName.Text,
                ...
                tbxInterest.Text, User.Identity.Name, banglaName, tbxDesignationMem.Text, Convert.ToDateTime(tbxJoinDate.Text), tbxBatch.Text);


        if (ChildEdited)
        {
            DataTable dtChilds = GetChilds();
            objMember.DeleteChilds(User.Identity.Name);
        ...
        if (EducationEdited)
        {
            ...
            objMember.DeleteEducation(User.Identity.Name);
        ...
        if (PostingPlaceEdited)
        {
            ...
            objMember.DeletePostingPlaces(User.Identity.Name);
    ...
    protected void btnDeleteAttachment_Click(object sender, EventArgs e)
    {
        if (hdnAttachmentName.Value != "")
        {
            ...
            //objNoticeBoard.Update(ID, Convert.ToDateTime(tbxDate.Text), txtBodyText.Text, "");
        }
    }
