...
public class MessageController
{
    ...
    private static void ShowMessage(string strMessage, string strTitle, string strCSSClassName, Page p)
    {
        ...
        ((Label)
            (((Panel) p.Master.FindControl("MessagePanel1").FindControl("pnlMessage")).FindControl("lblMessageTitle")))
            .Text = strTitle;
        ((Label)
            (((Panel) p.Master.FindControl("MessagePanel1").FindControl("pnlMessage")).FindControl("lblMessageDetails")))
            .Text = strMessage;
        ((Label)
            (((Panel) p.Master.FindControl("MessagePanel1").FindControl("pnlMessage")).FindControl("lblMessageDetails")))
            .CssClass = strCSSClassName;
    }
